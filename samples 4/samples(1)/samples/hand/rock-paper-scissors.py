...
HAND_INPUT_SIZE = np.array([224, 224])  # wh
...
def main():
    ...
    hand_model = cv.dnn.readNet('../models/handpose_estimation_mediapipe_2023feb.onnx')
...
def hand_postprocess(blob, rotated_palm_bbox, angle, rotation_matrix, conf_threshold=0.8):
    ...
    '''
    landmarks, conf, handness, landmarks_world = blob
    ...
    landmarks_world = landmarks_world[0].reshape(-1, 3) # shape: (1, 63) -> (21, 3)
    ...
    bbox = np.array([
        ...
        np.amax(landmarks[:, :2], axis=0)])  # [左上，右下]
    ...
    # [131]: confidence
    return np.r_[bbox.reshape(-1), landmarks.reshape(-1), rotated_landmarks_world.reshape(-1), handness[0][0], conf]
...
def getFingerBendings(handpose):
    ...
    bending1 = getFingerBending(landmarks_world[1:5,:])    # 大拇指
    bending2 = getFingerBending(landmarks_world[5:9,:])    # 食指
    bending3 = getFingerBending(landmarks_world[9:13,:])   # 中指
    bending4 = getFingerBending(landmarks_world[13:17,:])  # 无名指
    bending5 = getFingerBending(landmarks_world[17:21,:])  # 小拇指
...
def hand_visualize(image, hands, gestures):
    output = image.copy()

    for idx, handpose in enumerate(hands):
        conf = handpose[-1]
        bbox = handpose[0:4].astype(np.int32)
        landmarks_3d = handpose[4:67].reshape(21, 3).astype(np.int32)
        landmarks = landmarks_3d[:, 0:2]
