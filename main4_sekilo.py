...
# 移动用参数
center_deadzone = 70  # 中心点的死区范围
very_slow_speed = 5  # 非常慢速移动步长
slow_speed = 15  # 慢速移动步长
fast_speed = 30  # 快速移动步长

figure_xyz = [[[1,1,300],[1,1,300],[1,1,300],[1,1,300],[1,1,300],[1,1,300]]]
...
def give_back_figurexyz(cap,orbbec_cap,hands):
    ...
    dead_zone_radius = 70  # 中心死区半径
    slow_zone_radius = int(70 * 1.5)  # 慢速区半径
    fast_zone_radius = int(70 * 2.2)  # 快速区半径

    # 绘制死区圆
    cv2.circle(img, (int(center_x), int(center_y)), dead_zone_radius, (255, 255, 255), 2)
    # 绘制慢速移动区圆
    cv2.circle(img, (int(center_x), int(center_y)), slow_zone_radius, (255, 255, 0), 2)
    # 绘制快速移动区圆
    cv2.circle(img, (int(center_x), int(center_y)), fast_zone_radius, (0, 255, 0), 2)
...
def move_mouse(mouse, screen_width, screen_height):
    ...
    if is_allowed_click_gesture():
        ...
        # 根据相对位置移动鼠标
        if abs(centered_x) < center_deadzone and abs(centered_y) < center_deadzone:
            # 在中心点的死区范围内，不移动鼠标
            return True
        
        # 根据相对距离选择移动速度
        if abs(centered_x) >= center_deadzone:
            if abs(centered_x) >= 2.2 * center_deadzone :
                move_x = fast_speed
            elif abs(centered_x) >= 1.5 * center_deadzone :
                move_x = slow_speed
            else :
                move_x = very_slow_speed
            move_x *= 1 if centered_x > 0 else -1
        else:
            move_x = 0
        
        if abs(centered_y) >= center_deadzone:
            if abs(centered_y) >= 2.2 * center_deadzone :
                move_y = fast_speed
            elif abs(centered_y) >= 1.5 * center_deadzone:
                move_y = slow_speed
            else :
                move_y = very_slow_speed
            move_y *= 1 if centered_y > 0 else -1
        else:
            move_y = 0
        ...
        if DEBUG:
            print(f"Moving mouse to: {new_mouse_x}, {new_mouse_y}")
...
def is_click(current_frame):
    ...
    depth_threshold = 60  # 深度阈值
    move_threshold = 150  # 食指移动阈值
...
def press_T(keyboard):
    keyboard.press('t')
    time.sleep(0.1)
    keyboard.release('t')

def press_Y(keyboard):
    keyboard.press('y')
    time.sleep(0.1)
    keyboard.release('y')
